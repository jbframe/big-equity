...
## Options
1. **New Fastify container (`containers/simulationAPI`)** — dedicated HTTP service, mirrors the TS batch simulator's layout
2. **NestJS container** — batteries-included structure (DI, modules, decorators)
3. **Extend the TS batch simulator with an HTTP server** — reuse the existing container instead of adding one
...
- **Separation:** the TS simulator is a batch job (runs to completion, `restart: "no"`); bolting a server onto it would conflate two lifecycles. A separate container keeps each compose file honest
...
- **Consistency:** Same multi-stage Dockerfile pattern as the TS batch simulator — digest-pinned `node:24-alpine`, `npm ci`, compile with `tsc`, runtime stage ships `dist` only
...
- Fastify runtime deps mean the runtime stage needs production `node_modules`, unlike the batch simulator's dependency-free dist (`npm ci --omit=dev` in the runtime stage keeps it lean)
...
## Implementation
1. Scaffold `containers/simulationAPI`: `package.json` (`"type": "module"`), `tsconfig.json`, `src/` mirroring the TS batch simulator's conventions
...
4. Multi-stage Dockerfile from digest-pinned `node:24-alpine`: build stage compiles TypeScript; runtime stage copies `package.json` **and** `package-lock.json` (unlike the batch simulator's runtime stage, which skips the lockfile — `npm ci` needs it), runs `npm ci --omit=dev`, then adds `dist`
